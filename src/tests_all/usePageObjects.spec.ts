...
import {PageManager} from '../pages/page-objects/pageManager'
...
test('parametrized methods',async({page})=>{
    ...
    console.log('Zufälliger Name:', randomFullName)
    console.log('Zufällige Email:',randomEmail)

    await pm.navigateTo().formLayoutsPage()
    await pm.onFormLayoutsPage().submitUsingTheGridFormWithCredentialsAndSelectOption('test@test.com','welcome','Option 2')
 

    await pm.onFormLayoutsPage().submitInlineFormWithNameEmailAndCheckbox(randomFullName, randomEmail.replace(' ','').replace(' ',''), false)
