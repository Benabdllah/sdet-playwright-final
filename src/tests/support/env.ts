...
export interface EnvironmentConfig {
  ...
  features: {
    ...
    capturePassedVideos?: boolean;
    capturePassedTraces?: boolean;
  };
  retryDelay: number;
...
// URLs
export const BASE_URL = getOptionalEnvVar(
  "BASE_URL",
...
export const NAVIGATION_TIMEOUT = parseInt10(
...
export const SCREENSHOT = getOptionalEnvVar(
  "SCREENSHOT",
  "only-on-failure"
) as ScreenshotMode;
export const VIDEO = getOptionalEnvVar(
  "VIDEO",
  "retain-on-failure"
) as VideoMode;
...
export const COLOR_SCHEME = getOptionalEnvVar(
  "COLOR_SCHEME",
  "light"
) as ColorScheme;
...
export const ENABLE_PERFORMANCE = parseBoolean(
  process.env.ENABLE_PERFORMANCE,
  true
);
export const ENABLE_ACCESSIBILITY = parseBoolean(
  process.env.ENABLE_ACCESSIBILITY,
  false
);
export const ENABLE_VISUAL_REGRESSION = parseBoolean(
  process.env.ENABLE_VISUAL_REGRESSION,
  false
);
export const ENABLE_API_TESTING = parseBoolean(
  process.env.ENABLE_API_TESTING,
  true
);
...
// ================================
export const GEOLOCATION: Geolocation | undefined =
...
export const DEVICE_SCALE_FACTOR = parseFloat10(
...
export const CONTEXT_OPTIONS: BrowserContextOptions = {
  ...
  recordHar: RECORD_HAR
    ? ({
        path: path.resolve(process.cwd(), `artifacts/har/${Date.now()}.har`),
        mode: "minimal",
      } as any)
    : undefined,
  recordVideo:
...
export const REPORTING = {
  ...
  screenshotDir: getOptionalEnvVar("SCREENSHOT_DIR", "artifacts/screenshots"),
  videoDir: getOptionalEnvVar("VIDEO_DIR", "artifacts/videos"),
  traceDir: getOptionalEnvVar("TRACE_DIR", "artifacts/traces"),
  harDir: getOptionalEnvVar("HAR_DIR", "artifacts/har"),
...
export const CONFIG: EnvironmentConfig = {
  ...
  features: {
    ...
    capturePassedVideos: false, // Set to true to capture videos of passed tests
    capturePassedTraces: false, // Set to true to capture traces of passed tests
  },
...
if (ENV === "production" || ENV === "staging") {
  ...
  requiredVars.forEach((key) => {
    if (!process.env[key]) {
      console.warn(
        `⚠️  ${key} not explicitly set in ${ENV} environment – using default`
      );
    }
  });
  ...
  if (!HEADLESS) {
    console.warn(
      `⚠️  Running in HEADED mode in ${ENV} – this should be avoided in CI/CD`
    );
  }
}
