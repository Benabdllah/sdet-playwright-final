...
export class DataHelper {
  ...
  async loadData(
    ...
    const span = this.tracer.createSpan('loadData');
    ...
    try {
      ...
      if (span) this.tracer.endSpan(span);
  ...
  private async generateFieldValue(field: SchemaField): Promise<any> {
    ...
    if (field.enum && field.enum.length > 0) {
      return RandomUtil.pickRandom(field.enum);
    }
    ...
    switch (field.type) {
      case DataType.STRING:
        return RandomUtil.randomString({ minLength: 1, maxLength: field.maxLength || 10 });
      case DataType.NUMBER:
        return RandomUtil.randomFloat(0, 100);
      case DataType.INTEGER:
        return RandomUtil.randomNumber(0, 100);
      case DataType.BOOLEAN:
        return RandomUtil.randomBoolean();
      case DataType.EMAIL:
        return RandomUtil.randomEmail();
      case DataType.PHONE:
        return RandomUtil.randomPhone('+49-###-###-####');
      case DataType.URL:
        return RandomUtil.randomUrl();
      case DataType.DATE:
        return new Date().toISOString();
      case DataType.ARRAY:
        return [RandomUtil.randomString({ maxLength: 10 }), RandomUtil.randomString({ maxLength: 10 })];
      case DataType.OBJECT:
        return { value: RandomUtil.randomString({ maxLength: 10 }) };
      default:
        return null;
    }
  }
  ...
  private generateWithFaker(fakerPath: string): any {
    ...
    try {
      ...
      if (fakerPath.includes('person.name')) return RandomUtil.generatePersona().fullName;
      if (fakerPath.includes('internet.email')) return RandomUtil.randomEmail();
      if (fakerPath.includes('internet.url')) return RandomUtil.randomUrl();
      if (fakerPath.includes('phone.number')) return RandomUtil.randomPhone('+49-###-###-####');
      if (fakerPath.includes('date.')) return DateUtil.format(new Date());
      if (fakerPath.includes('company.name')) return RandomUtil.generatePersona().company;

      return RandomUtil.randomString({ maxLength: 10 });
    } catch (error) {
  ...
  private validateType(value: any, expectedType: DataType): boolean {
    switch (expectedType) {
      ...
      case DataType.EMAIL:
        return StringUtil.isEmail(value).valid;
      case DataType.PHONE:
        return StringUtil.isPhone(value).valid;
      case DataType.URL:
        return StringUtil.isUrl(value).valid;
      case DataType.ARRAY:
  ...
  getFromPool(poolName: string, index?: number): any {
    ...
    return RandomUtil.pickRandom(pool);
  }
  ...
  getFromPoolBatch(poolName: string, count: number): any[] {
    ...
    return RandomUtil.pickRandomMultiple(pool, Math.min(count, pool.length));
  }
