...
import { checkOrClickCheckbox } from '../utils/checkOrClickCheckbox-utils';
...
test('Liste aller Frames anzeigen', async ({ page }) => {
   ...
   // await listAllFrames(page); // 👉 zeigt alle vorhandenen Frames
    ...
    // console.log(html);
    await checkOrClickCheckbox(childFrame,"//div[@id='i6']//div[@class='AB7Lab Id5V1']" )
    //const child1=await childFrames.locator("//div[@id='i6']//div[@class='AB7Lab Id5V1']")//click({force:true})
    

   
   // await takescreen(page,'nestedframe')
