...
test.onl('handling table', async ({page }) => {
...
//         for (let j=0; j<await tds.count()-1;j++)// -1 -> wir brauche die letzte Spalte nicht
//         {
    ...
    for (let i=0; i<await rows.count();i++)
        {
           ...
           const tds=row.locator('td') 
                ...
                for (let j=1; j<await tds.count()-1;j++)// -1 -> wir brauche die letzte Spalte nicht
                {
                    console.log(await tds.nth(j).textContent())
                }
        }
