...
test('Liste aller Frames anzeigen', async ({ page }) => {
    ...
    const frame3= await page.frame({urlhttps://ui.vision/demo/webtest/frames/frame_3.html})

    //frame3.locator("input[name='mytext3']").fill('welcome')
    
    const childFrames=frame3.childFrames()
    await childFrames[0].locator("//div[@id='i6']//div[@class='AB7Lab Id5V1']//div[@class='rseUEf nQOrEb']").check({force:true})
