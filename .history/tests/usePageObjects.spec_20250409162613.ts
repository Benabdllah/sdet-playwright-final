...
test('parametrized methods',async({page})=>{
    ...
    const randomEmail= `${randomFullName}${faker.number.int(1000)}`

    await pm.navigateTo().formLayoutsPage()
    await pm.onFormLayoutsPage().submitUsingTheGridFormWithCredentialsAndSelectOption('test@test.com','welcome','Option 2')
    await pm.onFormLayoutsPage().submitInlineFormWithNameEmailAndCheckbox(randomFullName, 'mohamed@test.de', false)
    await pm.navigateTo().datepickerPage()
