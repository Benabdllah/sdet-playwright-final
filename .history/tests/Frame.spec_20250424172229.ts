...
test('Liste aller Frames anzeigen', async ({ page }) => {
   ...
   // await listAllFrames(page); // 👉 zeigt alle vorhandenen Frames
    ...
    const html = await childFrames[0].locator("//div[@id='i6']").innerHTML();
    console.log(html);

    const child1=await childFrames[0].locator("//div[@id='i6']//div[@class='AB7Lab Id5V1']")//click({force:true})
    await safeClick('childFrames',child1)
