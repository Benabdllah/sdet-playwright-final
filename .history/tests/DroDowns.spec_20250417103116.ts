...
test('handel dropDowns', async ({page}) => {
  ...
  *///5 select option from dropdown using loop
  ...
  for (const option of options2) {
    await page.locator('#country').checkcli()
    const attrValue = await option.textContent();
    if (attrValue?.match('India')) {
      
      await page.getByLabel('Country:').selectOption({label:attrValue})
      break;
    }
  }
