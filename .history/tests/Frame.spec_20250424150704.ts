...
test('Liste aller Frames anzeigen', async ({ page }) => {
    ...
    const frame3= switchToFrame(page,'frame_3')
    await frame3.await listAllFrames(page)
    await (await frame3).locator("input[name='mytext3']").fill('welcome')
   
    takescreen(page,'frame')
