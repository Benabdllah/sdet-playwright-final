...
test('handel inputbox', async ({page}) => {
  ...
  *///5 select option from dropdown using loop
  ...
  for (const option of options2) {
    const attrValue = await option.getAttribute('value');
    if (attrValue?.includes('9999999999999999999999999999999999999999999999999999999999')) {
      await page.selectOption('#country', attrValue);
      break;
    }
  }
