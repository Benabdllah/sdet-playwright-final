    ...
    async addProductToCart(productName: string) {
        ...
        for (const product of products) {
            ...
            if (name === productName) {
                ...
                // Klick + Navigation WARTEN
                await Promise.all([
                    this.page.waitForNavigation({ waitUntil: 'load' }),
                    product.doppelclick({ force: true }),
                ]);
        ...
        // Jetzt explizit warten, dass die neue Seite wirklich geladen ist
        await this.page.waitForSelector("//a[normalize-space()='Add to cart']", { state: 'visible', timeout: 15000 });

        const addToCartBtn = this.page.locator("//a[normalize-space()='Add to cart']");

        // Dialog nur EINMAL behandeln
